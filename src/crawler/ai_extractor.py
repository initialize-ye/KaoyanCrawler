...
- 成绩/分数线字段如果没有就设为null
- 如果页面同时包含招生人数（"计划招生"、"拟招"、"招生名额"等）和分数线信息，请在score_lines中一并提取enrollment字段
- **务必检查页面中是否有招生人数、复试人数、录取人数等数据**，这些经常出现在分数线表格旁边
- 只返回JSON，不要其他文字
"""
...
{{
  ...
  "subjects": [
    {{
      ...
      "subject3": "业务课一名称",
      "subject4": "业务课二名称",
      "enrollment": "招生人数（如有，整数）",
      "retest_score_line": "复试分数线（如有，数字）",
      "retest_count": "复试人数（如有，整数）",
      "retest_avg_score": "复试平均分（如有，数字）",
      "admission_count": "录取人数（如有，整数）",
      "admission_ratio": "复录比（如有，数字，如1.5表示复试人数/录取人数=1.5）",
      "admission_min_score": "录取最低分（如有，数字）",
      "admission_median_score": "录取中位数（如有，数字）",
      "admission_max_score": "录取最高分（如有，数字）",
      "admission_avg_score": "录取平均分（如有，数字）",
      "transfer_type": "调剂类型（如'接受调剂'、'不接受调剂'，无则留空）"
    }}
  ]
}}
...
- **务必提取所有能找到的数值字段**：招生人数、复试分数线、复试人数、录取人数、复录比、各分数统计等。如果页面中有这些数据但你不提取，就是遗漏
- 数值字段如果页面中没有则设为null，不要编造数字
- 注意区分"招生人数"/"计划招生"/"拟录取人数"等不同表述
- 注意区分"复试分数线"/"复试线"/"最低分"等不同表述
- 只返回JSON，不要其他文字
"""
